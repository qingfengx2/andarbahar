...
export class i18nMgr {
    ...
    public static _getSprite(path: string, cb: (spriteFrame: SpriteFrame) => void) {
        this.checkInit();
        resources.load("i18n/sprite/" + this.language + "/" + path, SpriteFrame, (err, spriteFrame: SpriteFrame) => {
            if (err) {
                return cb(null);
            }
            cb(spriteFrame);
    ...
    private static reloadLabel() {
        ...
        resources.load(url, (err, data: any) => {
            if (err) {
                console.error(err);
                this.labelData = {};
            } else {
